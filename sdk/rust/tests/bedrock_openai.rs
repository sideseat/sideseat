...
async fn test_bedrock_openai_chat_tool_use_loop() {
    ...
    server.mock(|when, then| {
        when.method(POST)
            ...
            .body_includes("tool_call_id");
        then.status(200)
            .header("content-type", "application/json")
            .body(OPENAI_COMPLETE_JSON);
    });
    ...
    let resp1 = provider
        .complete(vec![user_msg("Echo 'jackfruit'")], config.clone())
        .await
        .unwrap();
...
async fn test_bedrock_openai_responses_streaming_tools() {
    ...
    mock_sse(
        ...
        POST,
        "/responses",
        OPENAI_RESPONSES_STREAM_TOOL_EVENTS,
    );
    let mut config = default_config(BEDROCK_OPENAI_MODEL);
    config.tools = vec![echo_tool()];
