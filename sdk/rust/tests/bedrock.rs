//! Integration tests for native AWS Bedrock provider (IAM and API key variants).
//!
//! ```bash
...
async fn test_bedrock_complete() {
    ...
    let provider = BedrockProvider::from_env(region.clone()).await.expect("bedrock provider");
...
async fn test_bedrock_embed() {
    let region = bedrock_iam_env!();
    let provider = BedrockProvider::from_env(region).await.expect("bedrock provider");
...
async fn test_bedrock_vision() {
    ...
    assert!(!resp.text().is_empty(), "expected non-empty vision response");
    assert!(resp.usage.input_tokens > 0);
}
...
async fn test_bedrock_multi_image() {
    ...
    // Retry up to 3 times: model occasionally conflates identical images into 1.
    ...
    for attempt in 0..3u32 {
        ...
        let resp = retry(|| provider.complete(vec![msg.clone()], config.clone()))
            ...
            .unwrap();
        text = resp.text();
        if text.contains('2') || text.to_lowercase().contains("two") {
            break;
        }
    }
...
async fn test_bedrock_image_s3() {
    ...
    assert!(!resp.text().is_empty(), "expected non-empty image description from S3");
    assert!(resp.usage.input_tokens > 0);
}
...
async fn test_bedrock_document_s3() {
    ...
    let resp = provider.complete(vec![msg], config).await.unwrap();
    assert!(!resp.text().is_empty(), "expected non-empty doc summary from S3");
}
...
async fn test_bedrock_video_embedded() {
    ...
    let resp = provider.complete(vec![msg], config).await.unwrap();
    assert!(!resp.text().is_empty(), "expected non-empty video description");
    assert!(resp.usage.input_tokens > 0);
}
...
async fn test_bedrock_video_s3() {
    ...
    let resp = provider.complete(vec![msg], config).await.unwrap();
    assert!(!resp.text().is_empty(), "expected non-empty video description from S3");
    assert!(resp.usage.input_tokens > 0);
}
...
async fn test_bedrock_image_and_document() {
    ...
    assert!(!resp.text().is_empty(), "expected non-empty response to image+document request");
    assert!(resp.usage.input_tokens > 0);
}
...
async fn test_bedrock_prompt_caching_system() {
    ...
    assert!(!resp.text().is_empty(), "expected non-empty response with cached system prompt");
    assert!(resp.usage.input_tokens > 0);
}
...
async fn test_bedrock_prompt_caching_message() {
    ...
    assert!(!resp.text().is_empty(), "expected non-empty response with cached message");
    assert!(resp.usage.input_tokens > 0);
}
...
async fn test_bedrock_api_key_prompt_caching() {
    ...
    assert!(!resp.text().is_empty(), "expected non-empty response with api-key + caching");
    assert!(resp.usage.input_tokens > 0);
}
