...
async fn test_interactions_stateful_conversation() {
    ...
    server.mock(|when, then| {
        when.method(POST).path_includes("/interactions").body_includes("previous_interaction_id");
        then.status(200).header("content-type", "application/json").body(turn2_resp);
    });
    ...
    let resp1 = provider1.complete(vec![user_msg("My favourite colour is purple.")], config.clone()).await.unwrap();
    ...
    let resp2 = provider2.complete(vec![user_msg("What is my favourite colour?")], config).await.unwrap();
