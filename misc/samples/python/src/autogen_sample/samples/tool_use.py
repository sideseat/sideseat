...
async def temperature_forecast(city: str, days: int = 3) -> dict:
...
async def precipitation_forecast(city: str = "New York City", days: int = 3) -> str:
...
async def run_query(agent, team, query: str):
...
async def run(model_client, trace_attrs: dict):
    ...
    with tracer.start_as_current_span(
        ...
        await run_query(agent, team, "Provide a 3-day weather forecast for New York City.")

        # Reset team for next query
        await team.reset()
        ...
        await run_query(agent, team, "Provide a 7-day weather forecast for Los Angeles.")

        await team.reset()

        # Fourth call - cache hit (reads from cache)
        print("\n--- Fourth call (cache read) ---")
        await run_query(agent, team, "Provide a 14-day weather forecast for London.")
