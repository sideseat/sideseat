"""CLI argument parsing for Claude Agent SDK samples."""
...
_misc_dir = Path(__file__).parents[3]
load_dotenv(_misc_dir / ".env", override=True)

from config import DEFAULT_MODEL, MODEL_ALIASES, SAMPLES
...
def create_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="claude-agent-sdk",
        description="Run Claude Agent SDK samples with SideSeat instrumentation",
    )
    ...
    parser.add_argument(
        "--sideseat",
        action="store_true",
        help="Use SideSeat SDK for host-process spans and trace context propagation",
    )
