...
<Aside type="caution" title="Auth is on by default">
These commands assume a server started with `--no-auth`. Against a default server the MCP endpoint
requires `Authorization: Bearer <api-key>`, since it serves the project's prompts and spans — see the
[MCP Server guide](/docs/mcp/) for the config-file form that carries the header.
</Aside>
