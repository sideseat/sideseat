...
Supported `framework` values: `strands`, `langchain`, `langgraph`, `crewai`, `autogen`, `openai-agents`, `pydantic-ai`, `google-adk`, `agent-framework`, `bedrock`, `openai`, `anthropic`, `google-genai`, `vertex-ai`.
