...
The commands below carry no credential, so they work as written against a server started with
`--no-auth`. A default server requires `Authorization: Bearer <api-key>` on MCP as on any other read of
project data — this endpoint serves the project's prompts, spans and raw attributes. Add the header via
the config-file form in each tab (`"headers": { "Authorization": "Bearer $SIDESEAT_API_KEY" }`), and see
[Authentication](#authentication) below for how the key is scoped.
</Aside>
