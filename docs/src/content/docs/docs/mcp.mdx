...
A default server requires `Authorization: Bearer <api-key>` on MCP as on any other read of project data —
this endpoint serves the project's prompts, spans and raw attributes. Each tab below carries the credential
where its client supports one; set `SIDESEAT_API_KEY` first, or drop the credential if the server runs with
`--no-auth`. See [Authentication](#authentication) for how the key is scoped.
</Aside>
...
<Tabs>
  <TabItem label="Kiro / Kiro CLI">
    **CLI:**
    ...
    `kiro-cli mcp add` has no option for HTTP headers, so this form works against a server started
    with `--no-auth`. With auth enabled, use the config file.

    **Config file** — add to `.kiro/settings/mcp.json`:
  ...
  <TabItem label="Claude Code">
    **CLI:**
    ...
    Drop the `--header` if the server runs with `--no-auth`.
  ...
  <TabItem label="OpenAI Codex">
    **CLI:**
    ...
    `codex mcp add` takes no `--transport` flag — `--url` is what selects streamable HTTP. Drop
    `--bearer-token-env-var` if the server runs with `--no-auth`.
...
<Aside type="note">
Authentication follows the server's own setting. A server started with `--no-auth` needs no credential, and
SideSeat binds to `127.0.0.1` by default so only local tools can reach it. A server with auth enabled — which
is the default — requires an API key on MCP as on any other read of project data, since this endpoint serves
prompts, spans and raw attributes for the project in the URL:

```
Authorization: Bearer <api-key>
```

The key must belong to the organisation that owns that project; one from another organisation is refused.
</Aside>
...
|--------|----------|-------------|
| `setup_guide` | `framework` (optional) | Installation and instrumentation instructions for adding SideSeat to an AI application. Returns SDK one-liner and direct OTLP setup. |

Supported `framework` values.

Python: `strands`, `langchain`, `langgraph`, `crewai`, `autogen`, `openai-agents`,
`pydantic-ai`, `google-adk`, `agent-framework`, `claude-agent-sdk`, `bedrock`, `openai`,
`anthropic`, `google-genai`, `vertex-ai`.

TypeScript: `vercel-ai`, `strands-typescript`, `claude-agent-sdk-typescript`. These emit
`npm install` and TypeScript snippets instead of pip and Python.

Example prompt to your coding agent:

> Use the SideSeat `setup_guide` prompt with `framework=langchain` to add observability to this project.
...
To verify prompts are available, run `prompts/list` in the MCP inspector. You should see `setup_guide` listed.
