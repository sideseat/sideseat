...
- **Time-based**: If `retention.max_age_minutes` is set, data older than that is deleted
- **Volume-based**: If `retention.max_spans` is set, older spans are deleted first
