...
| `--clickhouse-url` | `SIDESEAT_CLICKHOUSE_URL` | - | ClickHouse 26.4+ connection URL |
...
| Option | Env Variable | Default | Description |
|--------|-------------|---------|-------------|
| `--queue-backend` | `SIDESEAT_QUEUE_BACKEND` | follows cache for compatibility | `memory`, `redis`, or `redpanda` |
| `--redpanda-brokers` | `SIDESEAT_REDPANDA_BROKERS` | `127.0.0.1:9092` | Comma-separated bootstrap brokers |
...
- [Overview](/docs/) — get started with SideSeat
