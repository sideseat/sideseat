...
class SideSeat:
    def __init__(
        ...
        *,                                  # every argument is keyword-only
        ...
        enable_logs: bool | None = None,    # None -> consult the environment
        capture_content: bool = True,
        encode_binary: bool = True,
        disabled: bool | None = None,       # None -> SIDESEAT_DISABLED
        debug: bool | None = None,          # None -> SIDESEAT_DEBUG
    ) -> None: ...
```
...
Unlike `span()`, this always starts a **new** trace: any currently active span is detached, so
the result is a root even when `trace()` is called inside another span. Use `span()` when you
want to nest into the surrounding trace instead.
...
Pending spans are flushed at interpreter exit, but by OpenTelemetry's own `TracerProvider`
(`shutdown_on_exit=True`), not by this method — `SideSeat.shutdown()` is registered with
`atexit` only when you add a file exporter, so that the file is closed. Call it explicitly, or
use the context manager, when you need the flush to happen at a point you control.
</Aside>
