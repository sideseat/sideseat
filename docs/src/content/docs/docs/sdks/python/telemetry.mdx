...
# Global: all spans get these IDs
client = SideSeat(user_id="user-123", session_id="session-abc")

# Per-operation: override for a specific scope
with client.span("handle-request", user_id="user-456", session_id="session-xyz"):
    run_agent()  # This span and children use overridden IDs
```
