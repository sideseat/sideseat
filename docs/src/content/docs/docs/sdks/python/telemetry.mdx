...
### `session(name, *, session_id, **kwargs)`

Create a session-scoped trace with an explicit session ID.

**Parameters**:
- `name` (str): Session name
- `session_id` (str, required): Session identifier
- `user_id` (str, optional): User ID for this session and its children

**Returns**: `ContextManager[Span]`

```python
with client.session("chat", session_id="sess-123", user_id="user-456"):
    bedrock.converse(modelId=model_id, messages=messages)
# All child spans get session.id="sess-123" and user.id="user-456"
```
...
Set `user_id` and `session_id` per-operation via `session()`, `trace()`, or `span()`:
...
with client.session("chat", session_id="session-abc", user_id="user-123"):
