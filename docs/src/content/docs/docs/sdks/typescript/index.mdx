...
```typescript
await init({ framework: Frameworks.VercelAI });  // Initializes
await init({ framework: Frameworks.VercelAI });  // No-op (already initialized)
```
...
The `@opentelemetry/api:` lines come from the OTel API registering its globals; SideSeat's own
...
In the second case `projectId` is not inserted — the project is taken from the path you
supplied. See [Configuration → endpoint](/docs/sdks/typescript/configuration/#endpoint).
...
- [Configuration](/docs/sdks/typescript/configuration/) — detailed configuration options
