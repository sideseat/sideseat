...
Shutdown the global instance, flushing pending spans and releasing resources. Resolves to whether the flush
completed: `false` means an exporter failed and those spans were not delivered, which is worth acting on if
you are draining before exit.
