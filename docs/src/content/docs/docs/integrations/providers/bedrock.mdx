...
- Python SDK installed (`pip install "sideseat[aws]"` or `uv add "sideseat[aws]"`) — the `aws` extra pulls in `wrapt`, without which Bedrock instrumentation silently does nothing
