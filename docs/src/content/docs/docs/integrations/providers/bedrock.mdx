...
Use `client.session()` to group multiple traces into a session. This sets `session.id` and `user.id` on all child spans, enabling session-level filtering and replay in the SideSeat UI.

A session typically contains multiple `client.trace()` calls, each representing a distinct interaction or topic:
...
with client.session("travel-planner", session_id="sess-abc", user_id="user-123"):
    ...
    # Trace 2: Food recommendations (fresh conversation)
...
```
travel-planner                         (session root, session.id + user.id)
  ├── trip-planning                    (trace 1)
  │   ├── chat claude-sonnet-4-5...    (turn 1)
  │   └── chat claude-sonnet-4-5...    (turn 2)
  └── food-recommendations             (trace 2)
      ├── chat claude-sonnet-4-5...    (turn 1)
      └── chat claude-sonnet-4-5...    (turn 2)
```

Each trace starts a fresh conversation with its own message history, while the session groups them for the same user.

`user_id` and `session_id` can also be passed directly to `client.trace()` when you don't need a session wrapper:
...
with client.trace("support-ticket", user_id="user-123", session_id="ticket-456"):
    bedrock.converse(modelId=model_id, messages=[...])
```
