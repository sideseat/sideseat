...
- Python SDK installed (`pip install "sideseat[aws]" boto3` or `uv add "sideseat[aws]" boto3`) — the `aws` extra pulls in `wrapt`, without which Bedrock instrumentation warns and produces no spans
...
- [Strands (Python)](/docs/integrations/frameworks/strands/) — use SideSeat with the Strands agent framework on Bedrock
