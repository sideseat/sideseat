...
The Secret Manager provides secure storage for sensitive data using native credential stores on each platform. Secrets are encrypted and protected by the operating system's security mechanisms.
...
```rust
use sideseat::core::{SecretManager, Secret, SecretKey, SecretMetadata};
use chrono::Utc;
