...
impl sideseat_ports::cache::CacheStore for CacheService {
    async fn get_raw(&self, key: &str) -> Result<Option<Vec<u8>>, String> {
        CacheService::get_raw(self, key)
            .await
    ...
    async fn set_raw(
        ...
        CacheService::set_raw(self, key, value, ttl)
            .await
            .map_err(|error| error.to_string())
    }
...
#[async_trait::async_trait]
impl sideseat_ports::cache::LocalCacheStore for CacheService {
    async fn get_local_raw(&self, key: &str) -> Result<Option<Vec<u8>>, String> {
        CacheService::get_local_raw(self, key)
            .await
            .map_err(|error| error.to_string())
    }
    ...
    async fn set_local_raw(
        ...
        CacheService::set_local_raw(self, key, value, ttl)
            .await
            .map_err(|error| error.to_string())
    }
