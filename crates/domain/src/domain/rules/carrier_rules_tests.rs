...
fn no_production_module_names_a_framework() {
    ...
    const EXEMPT: &[(&str, Allowed, &str)] = &[
        (
            "crates/domain/src/domain/traces/extract/framework_oracle.rs",
        ...
        (
            "crates/domain/src/domain/providers/service.rs",
            Allowed::Only(&["azure-ai-foundry"]),
...
///
/// The rules now compile in `crates/domain`, while their canonical source tree and architecture documents
...
fn production_rust_source_roots() -> Vec<std::path::PathBuf> {
    ...
    for entry in std::fs::read_dir(repository.join("crates")).expect("crates directory is readable")
    {
