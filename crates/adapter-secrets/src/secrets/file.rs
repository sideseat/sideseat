        ...
        match tokio::fs::read_to_string(path).await {
            Ok(json) => match serde_json::from_str::<SecretVault>(&json) {
                ...
                Err(e) => {
                    ...
                    let backup = format!("{}.corrupt.{}", path.display(), clock.now().timestamp());
    ...
    async fn set(&self, key: &SecretKey, secret: &Secret) -> Result<(), SecretError> {
        {
            ...
            vault.set_secret(key, secret, self.clock.now());
        }
        self.save().await
    }
...
mod tests {
    ...
    async fn test_file_provider_roundtrip() {
        ...
        let provider = FileProvider::init(dir.path(), test_clock()).await.unwrap();

        let key = SecretKey::global("test_key");
        let secret = test_secret("test_value");
    ...
    async fn test_created_at_preserved_on_update() {
        let dir = tempfile::tempdir().unwrap();
        let update_time = chrono::DateTime::from_timestamp(1_700_000_000, 0).unwrap();
        let provider = FileProvider::init(dir.path(), test_clock_at(update_time))
            .await
            .unwrap();

        let key = SecretKey::global("test_key");
        let secret = test_secret("v1");
        provider.set(&key, &secret).await.unwrap();
        ...
        assert_eq!(updated.metadata.updated_at, update_time);
    }
    ...
    async fn test_corruption_recovery() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join(FILE_SECRETS_FILENAME);
        let backup_time = chrono::DateTime::from_timestamp(1_700_000_000, 0).unwrap();
        ...
        assert_eq!(
            ...
            "secrets.json.corrupt.1700000000"
        );
    }
    ...
    async fn test_scoped_keys() {
        ...
        provider.set(&global, &test_secret("g")).await.unwrap();
