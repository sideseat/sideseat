...
use sideseat_ports::types::{NormalizedSpan, SearchField};
...
impl From<&NormalizedSpan> for SpanRow {
    fn from(span: &NormalizedSpan) -> Self {
        ...
        let ingested_at = chrono_to_time(span.ingested_at.unwrap_or(chrono::DateTime::UNIX_EPOCH));
        ...
        Self {
            ...
            search_completion_truncated: search_terms(span, SearchField::Completion).1,
            search_tool_name: search_terms(span, SearchField::ToolName).0,
            search_tool_name_truncated: search_terms(span, SearchField::ToolName).1,
            search_tool_args: search_terms(span, SearchField::ToolArgs).0,
            search_tool_args_truncated: search_terms(span, SearchField::ToolArgs).1,
            search_error: search_terms(span, SearchField::Error).0,
...
fn search_terms(span: &NormalizedSpan, field: SearchField) -> (Vec<String>, u8) {
    span.search.field(field).map_or_else(
        || (Vec::new(), 0),
