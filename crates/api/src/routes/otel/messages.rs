...
pub async fn get_trace_messages(
    ...
    let scoped = match scoped_tools {
        Some(scoped_tools) => Arc::new(scope_feed_to_trace(&reconstructed, scoped_tools, trace_id)),
        None => reconstructed,
    };

    // The window applies to the answer, after the whole session has been seen and narrowed to
    // this trace.
    let processed = feed_arc_after_window(scoped, from_timestamp, to_timestamp);
...
fn feed_arc_after_window(
    source: Arc<FeedResult>,
    from: Option<DateTime<Utc>>,
    to: Option<DateTime<Utc>>,
) -> Arc<FeedResult> {
    match apply_time_window(&source, from, to) {
        std::borrow::Cow::Borrowed(_) => source,
        std::borrow::Cow::Owned(filtered) => Arc::new(filtered),
    }
}
...
fn stream_messages_response(
    ...
    for block in &processed.messages {
        if start_time.is_none_or(|time| block.timestamp < time) {
            start_time = Some(block.timestamp);
        }
        if end_time.is_none_or(|time| block.timestamp > time) {
    ...
    let trailing = [
        ...
        (
            "tool_definitions",
            serde_json::to_string(&processed.tool_definitions),
        ),
        ("tool_names", serde_json::to_string(&processed.tool_names)),
    ...
    let len = processed.messages.len();
