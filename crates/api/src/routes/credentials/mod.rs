...
use crate::auth::{OrgAdmin, OrgRead};
use crate::extractors::ValidatedJson;
use crate::types::ApiError;
use sideseat_domain::providers::{CredentialConnectionTester, CredentialError, CredentialService};
...
pub struct CredentialsState {
    ...
    pub tester: Arc<dyn CredentialConnectionTester>,
}

/// Build Credentials routes
pub fn routes(
    service: Arc<CredentialService>,
    tester: Arc<dyn CredentialConnectionTester>,
) -> Router<()> {
    let state = CredentialsState { service, tester };
...
pub async fn list_credentials(
    ...
    let creds = state
        ...
        .list_for_org(
            &org.org_id,
            query.project_id.as_deref().map(ProjectId::from).as_ref(),
        )
        .await
...
pub async fn test_credential(
    ...
    let resolved = state
        .service
        .get_resolved(&org.org_id, &path.id)
        .await
        .map_err(credential_error)?
        .ok_or_else(|| credential_error(CredentialError::NotFound))?;
    let secret = state
        .service
        .get_secret(&org.org_id, &path.id)
        .await
        .map_err(credential_error)?;
    let result = state.tester.test(&resolved, secret.as_deref()).await;
...
pub async fn create_permission(
    ...
    let perm = state
        ...
        .create_permission(
            ...
            &path.id,
            &org.org_id,
            req.project_id.as_deref().map(ProjectId::from).as_ref(),
            &req.access,
            user_id,
        )
        .await
