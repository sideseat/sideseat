...
interface TokenTrendProps {
  data: Array<{ bucket: string; tokens: number }>;
  total: number;
  timeRange: TimeRange;
  isLoading?: boolean;
}

export function TokenTrend({ data, total, timeRange, isLoading }: TokenTrendProps) {
  ...
  const hasData = chartData.some((entry) => entry.tokens > 0);
      ...
      <Card className="h-full min-h-[280px]">
        ...
        <CardContent className="flex flex-1 items-center justify-center">
          <div className="text-sm text-muted-foreground text-center">
            No token usage for this period yet.
          </div>
        </CardContent>
      </Card>
    );
    ...
    <Card className="h-full min-h-[280px]">
      ...
      <CardContent className="flex flex-1 flex-col pt-2">
        ...
        <div className="mt-2 border-t pt-2 text-xs text-muted-foreground">
          Total: {formatCompact(total)} tokens
        </div>
      </CardContent>
    </Card>
  );
