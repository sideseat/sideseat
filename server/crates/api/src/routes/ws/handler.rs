                    ...
                    Message::Text(text) => {
                        ...
                        match parsed {
                            ...
                            InboundFrame::Pong(PongPayload { .. }) => {
                                ...
                                if let Some(cid) = cid
                                    && let Err(e) =
                                        state.registrations.touch(&cid, state.now_secs()).await
                                {
...
async fn recv_control(
    sub: &mut Option<sideseat_messaging::BroadcastTopicSubscriber<ConnectionControl>>,
...
mod control_routing_tests {
    use super::*;
    use chrono::Utc;
    use sideseat_adapter_registrations_memory::MemoryRegistrationStore;
