...
use crate::auth::{ProjectRead, ProjectWrite, TraceRead};
...
pub async fn delete_traces(
    ...
    let owner = acquire_deletion_fence(&state, &auth.project_id, "trace-delete").await?;
    let result = async {
        let repo = state.database.as_ref();
        let journal_bytes = body.trace_ids.iter().fold(0u64, |total, trace_id| {
        ...
        .await
    {
        tracing::warn!(
            project_id = %auth.project_id,
            %error,
            "Could not reconcile storage after trace deletion"
        );
    }
    state
        .storage_governance
