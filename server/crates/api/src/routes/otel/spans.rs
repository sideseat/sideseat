...
use crate::auth::{ProjectRead, ProjectWrite, SpanRead, TraceRead};
...
pub async fn delete_spans(
    ...
    let owner = acquire_deletion_fence(&state, &auth.project_id, "span-delete").await?;
    let result = async {
        let span_pairs = body
            .spans
            .iter()
            ...
            .collect::<Vec<_>>();
        let journal_bytes = span_pairs.iter().fold(0u64, |total, (trace_id, span_id)| {
        ...
        state
            .database
            .record_deleted_spans_journalled(&auth.project_id, &span_pairs)
            .await
            .map_err(ApiError::from_data)?;
        ...
        .await
    {
        tracing::warn!(
            project_id = %auth.project_id,
            %error,
            "Could not reconcile storage after span deletion"
        );
    }
    state
        .storage_governance
