...
async fn list_for_user_from_db(
    ...
    let rows = sqlx::query_as::<_, (String, String, String, String, i64, i64)>(
        ...
        WHERE om.user_id = ? AND o.deleting_at IS NULL
...
pub async fn update_organization(
    ...
    let result = sqlx::query(
        "UPDATE organizations SET name = ?, updated_at = ? WHERE id = ? AND deleting_at IS NULL",
    )
    .bind(name)
    .bind(now)
    .bind(id)
...
mod tests {
    ...
    async fn test_create_organization() {
        ...
        assert_eq!(org.created_at, TEST_NOW);
    ...
    async fn test_update_organization() {
        ...
        assert_eq!(updated.created_at, TEST_NOW);
        assert_eq!(updated.updated_at, TEST_NOW + 1);
    }
    ...
    async fn test_create_organization_with_owner() {
        ...
        let org =
            create_organization_with_owner(&pool, None, "New Org", "new-org", "local", TEST_NOW)
                .await
                .unwrap();
    ...
    async fn test_create_organization_with_owner_rollback_on_duplicate() {
        ...
        // Create org
        create_organization_with_owner(&pool, None, "First Org", "first", "local", TEST_NOW)
            .await
            .unwrap();

        // Try to create with same slug - should fail
        let result =
            create_organization_with_owner(&pool, None, "Second Org", "first", "local", TEST_NOW)
                .await;
