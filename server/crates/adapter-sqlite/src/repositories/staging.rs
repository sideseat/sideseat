//! SQLite staged-payload registry.
...
pub async fn create(pool: &SqlitePool, payload: &StagedPayload) -> Result<(), SqliteError> {
    let records = serde_json::to_string(&payload.records)
        .map_err(|error| SqliteError::Conflict(error.to_string()))?;
    sqlx::query(
...
pub async fn get(pool: &SqlitePool, id: &str) -> Result<Option<StagedPayload>, SqliteError> {
    let row = sqlx::query(
        "SELECT id, project_id, signal, blob_hash, byte_len, created_at, redrive_attempts,
                unconfirmed, records_json
         FROM staged_payloads WHERE id = ?",
    )
    .bind(id)
    .fetch_optional(pool)
    .await?;
...
pub async fn pending(pool: &SqlitePool, limit: usize) -> Result<Vec<StagedPayload>, SqliteError> {
    let rows = sqlx::query(
        "SELECT id, project_id, signal, blob_hash, byte_len, created_at, redrive_attempts,
                unconfirmed, records_json
...
pub async fn increment_attempts(pool: &SqlitePool, id: &str) -> Result<u32, SqliteError> {
...
pub async fn mark_unconfirmed(pool: &SqlitePool, id: &str) -> Result<(), SqliteError> {
    sqlx::query("UPDATE staged_payloads SET unconfirmed = 1 WHERE id = ?")
        .bind(id)
        .execute(pool)
        .await?;
    Ok(())
...
pub async fn delete(pool: &SqlitePool, id: &str) -> Result<(), SqliteError> {
...
fn parse_signal(value: &str) -> Result<StagedSignal, SqliteError> {
    match value {
        "traces" => Ok(StagedSignal::Traces),
        "metrics" => Ok(StagedSignal::Metrics),
        "logs" => Ok(StagedSignal::Logs),
        other => Err(SqliteError::Conflict(format!(
