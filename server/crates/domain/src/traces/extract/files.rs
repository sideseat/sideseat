...
struct FileCacheState {
    entries: HashMap<blake3::Hash, FileCacheEntry>,
    next_sequence: u64,
}
...
impl FileCacheState {
    ...
    fn purge_idle(&mut self, now: Instant) {
        let idle = Duration::from_secs(FILE_EXTRACTION_CACHE_IDLE_SECS);
...
impl Default for FileExtractionCache {
    fn default() -> Self {
        Self::new()
    }
}
...
impl FileExtractionCache {
    ...
    fn insert(&self, key: blake3::Hash, value: CachedFileResult) {
        ...
        if !state.entries.contains_key(&key)
            && state.entries.len() as u64 >= FILE_EXTRACTION_CACHE_MAX_ENTRIES
...
fn extract_or_cache(
    ...
    if let Some(cached) = cache_key.and_then(|key| cache.unwrap().get(&key)) {
        // A hit carries the bytes, and gets them the same way a miss does.
        //
        ...
        // *source text*, so a hit in project B says only that some project once hashed those bytes; B
        // may never have stored them. The same holds after a failed or quota-rejected write in the same
        // project.
        //
        // Reconstructing them from the source string was wrong for a `data:` URL, whose payload starts
        // after `data:image/png;base64,` - the whole URL would have been handed to a base64 decoder.
        // So the payload extraction runs on a hit too, and what the cache saves is the BLAKE3 over
        // megabytes, which is the expensive part.
