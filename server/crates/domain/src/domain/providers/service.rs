...
use sideseat_ports::types::{CredentialPermissionRow, CredentialRow, ProjectId};
...
/// Tests a resolved provider credential without coupling the API layer to a provider SDK.
#[async_trait]
pub trait CredentialConnectionTester: Send + Sync + 'static {
...
pub struct CredentialService {
    ...
    secrets: Arc<dyn CredentialSecretStore>,
...
impl CredentialService {
    ...
    pub async fn get_secret(
        ...
        let secret = self
            .secrets
            .get_credential_secret(org_id, cred_id)
            .await
    ...
    pub async fn create(
        ...
        if let Some(value) = secret_value
            && let Err(e) = self.secrets.put_credential_secret(org_id, &id, value).await
        {
    ...
    pub async fn delete(&self, id: &str, org_id: &str) -> Result<bool, CredentialError> {
        ...
        if deleted {
            ...
            let _ = self.secrets.delete_credential_secret(org_id, id).await;
