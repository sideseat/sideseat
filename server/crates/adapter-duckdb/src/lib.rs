...
use sideseat_ports::traits::{SpanStore, StorageGovernance, TransactionalRepository};
...
impl Clock for TestClock {
    fn now(&self) -> chrono::DateTime<chrono::Utc> {
        chrono::DateTime::from_timestamp(1_700_000_000, 0).expect("fixed test timestamp is valid")
    }
}
...
impl DuckdbService {
    ...
    pub async fn init(storage: &AppStorage, clock: Arc<dyn Clock>) -> Result<Self, DuckdbError> {
    ...
    async fn finish_retention_cleanup(
        analytics: &crate::DuckdbRepository,
            ...
            .await
        {
            Ok(spans) if !spans.is_empty() => {
                if let Err(e) = SpanStore::delete_spans(analytics, project_id, &spans).await {
                    clean = false;
                    tracing::warn!(
                        error = %e,
                        project_id = %project_id,
                        spans = spans.len(),
                        "Failed to re-apply journalled span deletions; retention cleanup stays recorded"
                    );
                }
            }
            Ok(_) => {}
            Err(e) => {
                ...
                tracing::warn!(
                    error = %e,
                    project_id = %project_id,
                    "Could not read journalled span deletions; retention cleanup stays recorded"
                );
            }
        }
                    ...
                    _ = interval.tick() => {
                            ...
                            Ok(claimed) if !claimed.is_empty() => {
                                ...
                                for (project_id, claimed) in &by_project {
                                    ...
                                    let owner = format!(
                                        "duckdb-cleanup:{}:{}",
                                        std::process::id(),
                                        db.clock.now().timestamp_micros()
                                    );
                                    let lock_now = db.clock.now();
                                        ...
                                        .await
                                    {
                                        ...
                                        Err(error) => {
                                            tracing::warn!(
                                                project_id = %project_id,
                                                %error,
                                                "Could not acquire retention-cleanup fence"
                                            );
                                            false
                                        }
                                    };
                                    ...
                                    let held = match governance
                                        .active_project_hold(&project_id, db.clock.now())
                                        .await
                                    {
                                        Ok(hold) => hold.is_some(),
                                        Err(error) => {
                                            // Uncertainty is resolved in the preservation direction.
                                            tracing::warn!(
                                                project_id = %project_id,
                                                %error,
                                                "Could not check legal hold before retention cleanup"
                                            );
                                            true
                                        }
                                    };
                                        ...
                                        .await
                                    {
                                        tracing::warn!(
                                            project_id = %project_id,
                                            %error,
                                            "Could not release retention-cleanup fence"
                                        );
                                    }
                                }
                            }
                        ...
                        for project_id in projects {
                                ...
                                .await
                            {
                                ...
                                Err(error) => {
                                    tracing::warn!(
                                        project_id = %project_id,
                                        %error,
                                        "Could not check legal hold before retention"
                                    );
                                    true
                                }
                            };
                            let retention_result = if held {
                                None
                            } else {
                                Some(
                                    db.run_project_retention(
                                        &config,
                                        quota_bytes,
                                        &project_id,
                                        &database,
                            ...
                            match retention_result {
                                None => {}
                                Some(Ok(result)) => {
                                    // Keep file/ref-count cleanup under the same project lease as the
                                    // analytical deletion. Otherwise a hold can begin after the span
                                    // transaction commits but before its already-recorded cleanup removes
                                    // associated bytes.
                                    if let Some(trace_ids) =
                                        result.trace_ids_by_project.get(project_id.as_str())
                                    {
                                        Self::finish_retention_cleanup(
                                            &DuckdbRepository(Arc::clone(&db)),
                                            &database,
    ...
    async fn run_project_retention(
        self: &Arc<Self>,
        config: &RetentionConfig,
        quota_bytes: u64,
        project_id: &ProjectId,
        ...
        tracing::debug!(project_id = %project_id, "Running retention check");
        let db = Arc::clone(self);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let record_intent = |by_project: &std::collections::HashMap<String, Vec<String>>| {
                for (project_id, trace_ids) in by_project {
                    ...
                    let written = handle
                        .block_on(database.record_retention_cleanup(&typed_project_id, trace_ids))
            ...
            let record_pressure = |project_id: &str, spans: &[(String, String)]| {
                use sideseat_ports::traits::{
                    DeletionCause, DeletionRecord, DeletionScope, retention_cleanup_logical_bytes,
                };

                let typed_project_id = ProjectId::from(project_id);
                let journal_bytes = spans.iter().fold(0u64, |total, (trace_id, span_id)| {
                ...
                let mut traces = spans
                    .iter()
                    .map(|(trace_id, _)| trace_id.as_str())
                    .collect::<Vec<_>>();
                traces.sort_unstable();
                traces.dedup();
                let cleanup_bytes = traces.iter().fold(0u64, |total, trace_id| {
                    total.saturating_add(retention_cleanup_logical_bytes(project_id, trace_id))
                });
                let additional = journal_bytes.saturating_add(cleanup_bytes);
                let reserved = handle
                    .block_on(governance.reserve_project_storage(
                        &typed_project_id,
                        additional,
                        quota_bytes,
                        now,
                ...
                if reserved.is_none() {
                    return Err(DuckdbError::Io(std::io::Error::other(format!(
                        "maintenance reserve exhausted while recording {} pressure evictions",
                        spans.len()
                    ))));
                }
                let written = handle
                    .block_on(database.record_pressure_eviction(&typed_project_id, spans))
                    .map_err(|e| DuckdbError::Io(std::io::Error::other(e.to_string())))?;
                tokens
                    .lock()
                    .unwrap_or_else(|e| e.into_inner())
