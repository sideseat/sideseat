//! DuckDB span repository using the Appender API.
//!
...
use crate::{DuckdbError, NormalizedSpan, in_transaction};
...
fn insert_spans(
    ...
    for span in spans {
        ...
        appender.append_row(params![
            ...
            timestamp_end,
            span.duration_ms,
            // Direct calls are test fixtures; production goes through `DuckdbRepository`, which stamps
            // the whole batch from its injected clock before this storage primitive is reached.
            SqlTimestamp(span.ingested_at.unwrap_or(chrono::DateTime::UNIX_EPOCH)),
            // GEN AI
