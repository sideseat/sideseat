//! Metrics ingestion: extract, fence, and write before answering.
//!
...
async fn ingest_inner(
    ...
    if let Some(governance) = governance {
        governance
            .stamp_metrics(&mut metrics)
            .await
            .map_err(|error| error.to_string())?;
    }
    let written_projects = metrics
        .iter()
        .map(|metric| ProjectId::from(metric.project_id.as_deref().unwrap_or(DEFAULT_PROJECT_ID)))
        .collect::<Vec<_>>();
