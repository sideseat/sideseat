...
def discover_strategy_ids(
    client, memory_id: str, namespace: str = "default"
) -> set[str]:
    ...
    try:
        ...
        return {
            r["memoryStrategyId"] for r in response.get("memoryRecordSummaries", [])
        }
    except Exception:
        return set()
...
def print_setup_help(client, memory_id: str | None):
    ...
    if not memory_id:
        ...
        print("  4. Set AGENT_CORE_MEMORY_ID in examples/.env")
        return
    ...
    if strategy_ids:
        ...
        for sid in sorted(strategy_ids):
            print(f"  - {sid}")
        print(
            "\nSet AGENT_CORE_MEMORY_STRATEGY_ID in examples/.env to one of these IDs"
        )
    else:
...
def create_session_manager(boto_session: boto3.Session) -> S3SessionManager | None:
    ...
    session_id = (
        f"{datetime.now(timezone.utc):%Y-%m-%d-%H-%M-%S}-{uuid.uuid4().hex[:8]}"
    )
...
def run(model, trace_attrs: dict):
    ...
    if not AGENT_CORE_MEMORY_ID or not AGENT_CORE_MEMORY_STRATEGY_ID:
        ...
        raise ValueError(
            "AGENT_CORE_MEMORY_STRATEGY_ID environment variable is required"
        )
