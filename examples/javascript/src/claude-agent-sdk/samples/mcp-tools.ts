...
// examples/javascript/src/claude-agent-sdk/samples -> tools/mcp-calculator
...
export async function run(modelId: string, env: Record<string, string>) {
  if (!fs.existsSync(path.join(MCP_SERVER_DIR, 'calculator.py'))) {
    throw new Error(
      `MCP server not found in ${MCP_SERVER_DIR}. Run from the examples/javascript directory.`
    );
  }
  ...
  const options = buildOptions(modelId, env, {
    mcpServers: {
      calculator: {
        type: 'stdio',
        // Launch via uv: tools/mcp-calculator has its own venv with fastmcp, so a bare
