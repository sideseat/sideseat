...
// Content path: examples/javascript/src/strands/samples -> examples/assets
...
export async function run(modelId: string) {
  ...
  if (!fs.existsSync(imgPath)) {
    throw new Error(`Image not found: ${imgPath}. Run from examples/javascript directory.`);
  }
  if (!fs.existsSync(pdfPath)) {
    throw new Error(`PDF not found: ${pdfPath}. Run from examples/javascript directory.`);
  }
