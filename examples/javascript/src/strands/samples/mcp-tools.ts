...
export async function run(modelId: string) {
  if (!fs.existsSync(path.join(MCP_SERVER_DIR, 'calculator.py'))) {
    throw new Error(
      `MCP server not found in ${MCP_SERVER_DIR}. Run from examples/javascript directory.`
    );
  }

  const calculatorTools = new McpClient({
    // Launch via uv, matching the claude-agent-sdk suite: tools/mcp-calculator has its own venv with
