...
Create `examples/.env` (shared with Python samples):
...
**mcp-tools** (Strands only): Connects to an MCP server (`tools/mcp-calculator/calculator.py`) via stdio transport.
...
**MCP server not found**: Run from `examples/javascript` directory, ensure Python is in PATH.

**Image/PDF not found**: Run from `examples/javascript` directory. Content files are in `examples/assets/`.
